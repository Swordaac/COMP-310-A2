...
#include <limits.h>  // For general limits
// or
#include <unistd.h>  // For POSIX-specific limits
...
char* copy_script_to_backing_store(const char* originalScriptPath) {
    // Generate a unique file name in the backing store
    // For simplicity, using a timestamp or a unique identifier
    char newFilePath[PATH_MAX];
    snprintf(newFilePath, sizeof(newFilePath), "./backing_store/%s_%ld", originalScriptPath, time(NULL));

    // Open the original script file
    FILE* originalFile = fopen(originalScriptPath, "r");
    if (!originalFile) {
        perror("Failed to open original script file");
        return NULL;
    }

    // Create a new file in the backing store
    FILE* newFile = fopen(newFilePath, "w");
    if (!newFile) {
        perror("Failed to create new script file in backing store");
        fclose(originalFile);
        return NULL;
    }
    ...
    // Return the new path for execution
    // Note: This is a simplification. You might need dynamic memory allocation here.
    return strdup(newFilePath);
}
...
int run(char* script){
	...
	// Copy script to backing store and get new script path
    char* newScriptPath = copy_script_to_backing_store(script);
    if(newScriptPath == NULL) {
        // Handle error if copying failed (e.g., file does not exist)
        errCode = 11; // Assuming 11 is the error code for file not found
